...
int main(int argc, char *argv[]) {
  ...
  MPI_Comm_size(MPI_COMM_WORLD, &p); // récupération du nombre de processus    
	...
	if (rank == MASTER) {
		...
		h = r.file.ras_height;
		w = r.file.ras_width;
		/* test dimension image locale */ 
		if (h % p != 0) {
			printf("erreur : la hauteur = %d n'est pas un multiple de p = %d !\n", h, p);
			MPI_Finalize();
			return 0;
		}
	}
	...
	unsigned char *ima_premier;
		ima_premier = (unsigned char*) malloc( 5 * w * sizeof(unsigned char));
		if (ima_premier == NULL) {
			printf("Erreur dans l'allocation de tmp dans convolution \n");
			return 1;
		}
	unsigned char *ima_dernier;
		ima_dernier = (unsigned char*) malloc( 5 * w * sizeof(unsigned char));
		if (ima_dernier == NULL) {
			printf("Erreur dans l'allocation de tmp dans convolution \n");
			return 1;
	}
	...
	// Envoie blocs
	MPI_Scatter(r.data, w*h/p, MPI_UNSIGNED_CHAR, ima_loc + (rank > 0 ? w:0), w*h/p, MPI_UNSIGNED_CHAR, MASTER, MPI_COMM_WORLD);
	MPI_Request request;	

  /* La convolution a proprement parler */
  for(i=0 ; i < nbiter ; i++){ 
	  
		memcpy( ima_loc+w, ima_dernier+2*w, w*sizeof(unsigned char));
		memcpy( ima_loc+w*2, ima_premier+3*w, w*sizeof(unsigned char));
		memcpy( ima_loc+w*(h_loc-2), ima_dernier+w, w*sizeof(unsigned char));
		memcpy( ima_loc+w*(h_loc-1), ima_dernier+2*w, w*sizeof(unsigned char));
		
		// Envoi des données manquantes :
		if (rank > 0) { 
			MPI_Isend(ima_loc + w *2,w,MPI_UNSIGNED_CHAR,rank - 1,0,MPI_COMM_WORLD,&request);
		}
		if (rank < p-1) {
			MPI_Isend(ima_loc + w *(h_loc - 2),w,MPI_UNSIGNED_CHAR,rank + 1,0,MPI_COMM_WORLD,&request);
		}
		
		// Convolution du bloc de 1 à h_loc-1
		convolution( filtre, ima_loc, h_loc-1, w); 
		
		// Reception des données manquantes :
		if (rank > 0) { 
			MPI_Irecv(ima_premier + w, w ,MPI_UNSIGNED_CHAR,rank-1,0,MPI_COMM_WORLD,&request);
			
		}
		if (rank < p-1) {
			MPI_Irecv(ima_dernier + w * 3 ,w,MPI_UNSIGNED_CHAR,rank + 1,0,MPI_COMM_WORLD,&request);
		}
		MPI_Wait(&request,&status);
		
		// Convolution de la ligne 1
		convolution( filtre, ima_premier + w, 3, w);
		// Convolution de la ligne h_loc-1
		convolution( filtre, ima_dernier + w, 3, w);
		
		// Complète ima_loc
		for (k=0; k<w; k++){
			ima_loc[w+k]=ima_premier[2*w+k];
			ima_loc[w*(h_loc-1)+k]=ima_dernier[w+k];
		}
  ...
  fin = my_gettimeofday();
  printf("Temps processus : %g seconde(s) \n", fin - debut);
