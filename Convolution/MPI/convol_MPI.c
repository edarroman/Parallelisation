...
int main(int argc, char *argv[]) {
  ...
  for(i=0 ; i < nbiter ; i++){
		// Echange des données manquantes :
		if (rank > 0) { 
			// Premiere ligne
			MPI_Send(ima_loc + w, w, MPI_CHAR, rank-1, 0, MPI_COMM_WORLD);
			MPI_Recv(ima_loc, w, MPI_CHAR, rank-1, 0, MPI_COMM_WORLD, &status_avant);
		}
		if (rank < p-1) { 
			// Dernière ligne
			MPI_Send(ima_loc + (h_loc - 2) * w , w, MPI_CHAR, rank+1, 0, MPI_COMM_WORLD);	
			MPI_Recv(ima_loc + (h_loc - 1) * w , w, MPI_CHAR, rank+1, 0, MPI_COMM_WORLD, &status_apres);
		}
